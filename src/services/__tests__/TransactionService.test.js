...
describe('TransactionService', () => {
  ...
  describe('addTransaction', () => {
    ...
    test('should add a new savings transaction', () => {
      ...
      expect(result.data.addedInterest).toBe(false);
      expect(result.data.transactions[1]).toEqual(
        expect.objectContaining({
          amount: 200,
          note: 'Second deposit',
          runningTotal: 700
    ...
    test('should add interest and payment for debt mode', () => {
      ...
      expect(result.data.addedInterest).toBe(true);
      ...
      expect(result.data.transactions[2]).toEqual(
        expect.objectContaining({
          amount: -100,
          type: 'transaction'
        })
  ...
  describe('deleteTransaction', () => {
    const transactions = [
      { id: 1, amount: 100, runningTotal: 100 },
      { id: 2, amount: 50, runningTotal: 150 },
      { id: 3, amount: -25, runningTotal: 125 }
    ];
    ...
    test('should indicate when to reset interest date', () => {
      const singleTransaction = [{ id: 1, amount: 100, runningTotal: 100 }];
  ...
  describe('recalculateRunningTotals', () => {
    test('should recalculate running totals correctly', () => {
      const transactions = [
        { id: 1, amount: 100, runningTotal: 0 },
        { id: 2, amount: 50, runningTotal: 0 },
        { id: 3, amount: -25, runningTotal: 0 }
      ];
  ...
  describe('calculateCurrentTotal', () => {
    test('should calculate total from transactions', () => {
      ...
      const result = TransactionService.calculateCurrentTotal(transactions);
  ...
  describe('validateTransactions', () => {
    test('should validate correct transactions array', () => {
      ...
      const result = TransactionService.validateTransactions(transactions);
      expect(result.isValid).toBe(true);
    });

    test('should fail validation for non-array', () => {
      const result = TransactionService.validateTransactions('not an array');
